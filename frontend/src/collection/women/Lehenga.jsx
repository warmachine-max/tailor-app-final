    ...
    const handleCreateSection = async (e) => {
        ...
        try {
          ...
          const res = await fetch(`${API_URL}/api/women/lehenga/create`, {
            ...
            setItems((prev) => [data.lehenga, ...prev]); 
        ...
        <div className="bg-gray-50 min-h-screen py-16 px-4">
            <div className="max-w-7xl mx-auto space-y-12">
                ...
                {loading ? (
                    ...
                    <p className="text-center text-gray-500 py-10">No lehengas available in stock.</p>
                ) : (
                        ...
                        {sortedItems.map((it, idx) => (
                            ...
                            >
                                <div className="w-full lg:w-1/2 space-y-4">
                                    <span className="text-xs font-medium uppercase text-purple-500 tracking-widest">Bridal & Festive Collection</span>
                                    <h2 className="text-4xl font-extrabold text-gray-900 tracking-tight">{it.title}</h2>
                                    <p className="text-gray-700 leading-relaxed italic border-l-4 border-purple-200 pl-3">{it.notes || "Exquisite Lehenga Choli with fine detailing and traditional embroidery."}</p>
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-4 z-50">
                    <form onSubmit={handleCreateSection} className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl space-y-6 relative">
                        <div className="flex justify-between items-center border-b pb-3 mb-4">
                            <h3 className="text-2xl font-extrabold text-purple-900">Add New Lehenga to Stock</h3>
                        ...
                        <input type="text" placeholder="Lehenga Title (e.g., Silk Bridal Choli)" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border border-gray-300 p-3 rounded-xl focus:ring-4 focus:ring-purple-200 focus:border-purple-500 focus:outline-none transition" required />
                        <textarea placeholder="Detailed Description / Embroidery Notes" value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full border border-gray-300 p-3 rounded-xl focus:ring-4 focus:ring-purple-200 focus:border-purple-500 focus:outline-none transition" rows={4} />
                        <div className="flex gap-4 items-center">
                            <FaTag className="text-purple-600 w-5 h-5 flex-shrink-0" />
                            <input type="number" placeholder="Price (e.g., 25000)" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full border border-gray-300 p-3 rounded-xl focus:ring-4 focus:ring-purple-200 focus:border-purple-500 focus:outline-none transition" />
                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                            ...
                            <button type="submit" disabled={uploading} className="px-5 py-2 bg-purple-900 text-white rounded-xl hover:bg-purple-800 transition disabled:opacity-60 font-semibold focus:ring-4 focus:ring-purple-300">
                                {uploading ? "Uploading..." : "Create Lehenga"}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-4 z-50">
                    <form onSubmit={handleBookingSubmit} className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl space-y-6 relative">
                        ...
                        <div className="bg-purple-50 p-3 rounded-xl border border-purple-200">
                            <p className="font-semibold text-purple-800">{bookingProduct?.title || 'Selected Lehenga'}</p>
