...
export default function Kurta() {
    ...
    // NOTE: Updated PLACEHOLDER for better visibility
    const PLACEHOLDER = "/mnt/data/kurta-placeholder.png";
    ...
    const sortedItems = useMemo(() => {
        ...
        switch (sortOrder) {
            case 'price_asc':
                // Sort ascending by price (lowest first), treating null/undefined as 0
                return sortableItems.sort((a, b) => (parseFloat(a.price) || 0) - (parseFloat(b.price) || 0));
            case 'price_desc':
                // Sort descending by price (highest first)
                return sortableItems.sort((a, b) => (parseFloat(b.price) || 0) - (parseFloat(a.price) || 0));
            case 'default':
            default:
                // Sort by creation date (newest first). Assumes creation date is available or uses a default fallback.
                return sortableItems.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
        }
    }, [items, sortOrder]);
    ...
    const handleCreateKurta = async (e) => {
        ...
        try {
            ...
            alert(err.message || "Failed to create kurta");
        } finally {
            setUploading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            alert(err.message || "Failed to delete kurta");
        }
    };
        ...
        <div className="bg-gray-50 min-h-screen py-16 px-4">
            <div className="max-w-7xl mx-auto space-y-12">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-purple-200">
                    <h1 className="text-5xl font-extrabold text-purple-900 tracking-tight flex items-center">
                        <FaTshirt className="inline mr-3 text-purple-600 w-8 h-8" /> Kurta Collection 
                    </h1>
                    <div className="flex gap-4 items-center">
                        ...
                        <div className="relative inline-block text-left">
                            ...
                            >
                                ...
                                <span className="sm:hidden">{sortOrder === 'default' ? 'Sort' : 'Sorted'}</span>
                ...
                {/* Loading / Items */}
                {loading ? (
                    <p className="text-center text-gray-600 py-10">Loading kurtas...</p>
                ) : sortedItems.length === 0 ? ( // <-- Using sortedItems here too for consistency
                    <p className="text-center text-gray-500 py-10 text-xl">No kurtas available in the collection.</p>
                ) : (
                    <div className="space-y-16">
                        {sortedItems.map((it, idx) => (
                            <section
                                key={it._id}
                                className={`flex flex-col lg:flex-row items-center gap-10 p-8 bg-white rounded-3xl shadow-xl hover:shadow-2xl transition duration-300 ${
                                    idx % 2 === 1 ? "lg:flex-row-reverse" : ""
                                }`}
                            >
                                {/* Text */}
                                <div className="w-full lg:w-1/2 space-y-4">
                                    <span className="text-xs font-medium uppercase text-purple-500 tracking-widest">Men's Ethnic Wear</span>
                                    <h2 className="text-4xl font-extrabold text-gray-900 tracking-tight">{it.title}</h2>
                                    <p className="text-gray-700 leading-relaxed italic border-l-4 border-purple-200 pl-3">{it.notes || "Elegant kurta designed for style and comfort, perfect for all occasions."}</p>
                                    ...
                                    <div className="text-4xl font-extrabold text-green-700 flex items-center pt-4">
                                        ...
                                        {it.price ? `Rs. ${parseFloat(it.price).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : 'Enquire for Price'}
                                    </div>
            ...
            {showAdminForm && isAdmin && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-4 z-50">
                    <form
                        ...
                        className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl space-y-6 relative"
                    >
                        <div className="flex justify-between items-center border-b pb-3 mb-4">
                            <h3 className="text-2xl font-extrabold text-purple-900">Add New Kurta to Stock</h3>
                        ...
                        <input
                            type="text"
                            placeholder="Kurta Title (e.g., Linen Pathani Kurta)"
                            value={title}
                        ...
                        />
                        <textarea
                            placeholder="Detailed Description / Fabric Notes"
                            value={notes}
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                            ...
                            >
                                {uploading ? "Uploading..." : "Create Kurta"}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-4 z-50">
                    ...
                    >
                        ...
                        <div className="bg-purple-50 p-3 rounded-xl border border-purple-200">
                            <p className="font-semibold text-purple-800">{bookingProduct.title}</p>
                            <p className="text-sm text-gray-600 mt-1">Price: {bookingProduct.price ? `Rs. ${parseFloat(bookingProduct.price).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : 'On Request'}</p>
                        </div>
                        ...
                        />
                        <textarea
                            placeholder="Specific request or questions (optional)"
                            value={bookNotes}
