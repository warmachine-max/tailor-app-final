...
export default function Suit() {
    ...
    useEffect(() => {
        ...
        const fetchItems = async () => {
            try {
                const res = await fetch(`${API_URL}/api/men/suits`);
    ...
    const sortedItems = useMemo(() => {
        ...
        switch (sortKey) {
            ...
            case 'none':
            default:
                // No sort applied (maintains fetch order)
                break;
        }
        return sorted;
    }, [items, sortKey]); 
    ...
    const handleCreateSection = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`${API_URL}/api/men/suit/create`, {
                method: "POST",
                credentials: "include",
                body: formData,
            });
    ...
    const handleDelete = async (id) => {
        ...
        try {
         const res = await fetch(`${API_URL}/api/men/suit/${id}`, {
                method: "DELETE",
                credentials: "include",
            });
            const data = await res.json();
            ...
            <div className="max-w-7xl mx-auto space-y-16">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-purple-200">
                    <h1 className="text-4xl font-extrabold text-purple-900 tracking-tight">
                        <FaTshirt className="inline mr-3 text-purple-600" /> Men's Suit Collection
                    </h1>

                    {/* Sort Control & Admin Button Group */}
                    <div className="flex items-center gap-4 flex-wrap justify-end">
                        
                        {/* Sort Dropdown - FIXED: Removed the conditional check items.length > 1 */}
                        {items.length > 0 && (
                ...
                {loading ? (
                    <p className="text-center text-gray-600 py-10">Loading suits...</p>
                ) : items.length === 0 ? (
                    <p className="text-center text-gray-500 py-10 text-xl">No suits available in the collection.</p>
                ) : (
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-4 z-50">
                    <form className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl space-y-6 relative" onSubmit={handleCreateSection}>
                        <div className="flex justify-between items-center border-b pb-3 mb-4">
                            <h3 className="text-2xl font-extrabold text-purple-900">Add New Suit</h3>
                        ...
                        <input type="text" placeholder="Suit Title (e.g., Casual Linen Suit)" value={title} onChange={(e) => setTitle(e.target.value)}
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                            ...
                            <button type="submit" disabled={uploading}
                                ...
                                {uploading ? "Uploading..." : "Create Suit"}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            {showBookingModal && bookingProduct && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center px-4 z-50">
                    <form className="bg-white rounded-3xl p-8 w-full max-w-lg shadow-2xl space-y-6 relative" onSubmit={handleBookingSubmit}>
                        <div className="flex justify-between items-center border-b pb-3 mb-4">
                            <h3 className="text-2xl font-extrabold text-purple-900">Book Consultation for Suit</h3>
